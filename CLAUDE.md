...
**Data changes (CSV → JSON):** Run `node scripts/programmes-csv-to-json.js` and `node scripts/exhibitors-csv-to-json.js`, then commit the output files in `data/`.
...
| `public.teams` | `id`, `lead_user_id`, `invite_token`, `max_members` (5) |
...
**Update session/exhibitor data** → edit CSVs, run `node scripts/programmes-csv-to-json.js` + `node scripts/exhibitors-csv-to-json.js`, commit JSON files
