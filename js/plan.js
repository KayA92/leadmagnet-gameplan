...
let _planEditorBucket    = new Set(['top', 'high', 'medium', 'neutral']);
...
function renderTabNav() {
  ...
  const tabs = [
    ...
    {
      id: 'team', label: 'Team', badge: teamCount || null,
...
window.planDownloadCpd = function() {
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8">
    ...
    <style>
      ...
      /* Table layout note: Session column is the value column and must
         hold its width when the print preview shrinks. We give it a
         fluid width via 'width: auto' (effectively the leftover) plus
         a sensible min-width, while the When + Hours columns are
         capped. The When column is allowed to wrap so it shrinks
         vertically rather than squashing Session sideways. */
      table { width: 100%; border-collapse: collapse; table-layout: fixed; }
      th, td { padding: 12px 8px; text-align: left; border-bottom: 1px solid #ececf3; vertical-align: top; word-wrap: break-word; overflow-wrap: break-word; }
      ...
      th:first-child, td:first-child { width: auto; min-width: 240px; }
      ...
      td.meta, th.meta { color: #6b6b80; font-size: 9pt; font-family: 'JetBrains Mono', 'SF Mono', Menlo, monospace; letter-spacing: 0.06em; text-transform: uppercase; width: 130px; line-height: 1.45; }
      td.hrs, th.hrs { text-align: right; width: 64px; font-variant-numeric: tabular-nums; font-family: 'JetBrains Mono', 'SF Mono', Menlo, monospace; }
    ...
    <table>
      <thead><tr><th>Session</th><th class="meta">When</th><th class="hrs">CPD hrs</th></tr></thead>
...
// URL where /images/* won't resolve. Light-bg variant straight from
// the brand SVG: black 'WORKIRO' glyphs, deep-purple subtitle, teal
// accent tag. Sized via inline style so it sits cleanly on print.
const _WORKIRO_LOGO_INLINE = `<svg viewBox="0 0 420 130" xmlns="http://www.w3.org/2000/svg" style="width:128px;height:auto;display:block;">
  <path d="M26.1574 104.397H31.8161L21.1664 129H15.5428L4.9283 104.397H11.0791L18.5655 121.971L26.1574 104.397ZM40.4347 107.455C38.3961 107.455 37.0253 106.119 37.0253 104.397C37.0253 102.675 38.3961 101.339 40.4347 101.339C42.4732 101.339 43.844 102.604 43.844 104.291C43.844 106.119 42.4732 107.455 40.4347 107.455ZM37.6931 129V110.091H43.1762V129H37.6931ZM57.368 112.586C58.7036 110.758 60.9531 109.809 63.8703 109.809V114.871C63.4134 114.836 63.0619 114.8 62.6402 114.8C59.6175 114.8 57.6141 116.452 57.6141 120.073V129H52.1311V110.091H57.368V112.586ZM82.3689 124.22L83.8451 128.086C82.7203 128.895 81.0684 129.281 79.3813 129.281C74.9176 129.281 72.3167 126.997 72.3167 122.498V114.73H69.3995V110.512H72.3167V105.908H77.7997V110.512H82.5095V114.73H77.7997V122.427C77.7997 124.044 78.6784 124.923 80.1546 124.923C80.963 124.923 81.7714 124.677 82.3689 124.22ZM104.356 110.091H109.839V129H104.637V126.751C103.196 128.402 101.087 129.281 98.7676 129.281C94.0227 129.281 90.6837 126.61 90.6837 120.775V110.091H96.1667V119.967C96.1667 123.13 97.5726 124.536 99.9978 124.536C102.528 124.536 104.356 122.919 104.356 119.44V110.091ZM126.412 109.809C132.176 109.809 135.515 112.481 135.515 118.21V129H130.384V126.645C129.364 128.367 127.396 129.281 124.619 129.281C120.191 129.281 117.555 126.821 117.555 123.552C117.555 120.213 119.91 117.893 125.674 117.893H130.032C130.032 115.539 128
 70.8594L197.269 86.4693H211.256L198.754 68.5282C198.79 68.5127 198.832 68.5016 198.868 68.4861ZM194.152 43.6293C196.124 45.3343 197.109 47.7321 197.109 50.8225C197.109 53.8597 196.124 56.2575 194.152 58.0158C192.181 59.7209 189.197 60.5734 185.2 60.5734H174.65V41.0716H185.2C189.197 41.0716 192.181 41.9242 194.152 43.6293Z" fill="black"/>
  <path d="M271.618 30.5215H257.231L232.454 56.6305V30.5215H219.586V86.4693H232.454V72.3469L239.952 64.5209L257.87 86.4693H272.976L248.455 55.4493L271.618 30.5215Z" fill="black"/>
  <path fill-rule="evenodd" clip-rule="evenodd" d="M140.153 0.00721338L100.829 7.47358C100.463 7.5424 100.266 8.11742 100.388 8.75461L101.813 16.2565C101.935 16.8959 102.328 17.3577 102.694 17.2866L142.018 9.82028C142.384 9.75145 142.582 9.17644 142.46 8.53925L141.034 1.03736C140.912 0.397961 140.519 -0.0638314 140.153 0.00721338Z" fill="#008299"/>
</svg>`;

// Workiro CTA block — opens with a one-line hook, follows with the
// Workiro pitch, closes with a CTA link to workiro.com. Same block
// is used at the bottom of the debrief PDF and the CPD log PDF so
// the two read as a matched pair when printed together.
function _workiroPdfFooterHtml() {
  return `
    <section class="wk-foot">
      <div class="wk-foot-logo">${_WORKIRO_LOGO_INLINE}</div>
      <p class="wk-foot-hook"><strong>This PDF will get lost.</strong></p>
      <p class="wk-foot-pitch">Downloads folder. Forwarded once. Re-uploaded somewhere. Asked for again in 18 months. That's the problem we built Workiro to solve.</p>
      <p class="wk-foot-cta">Cloud document management for UK accountants — trusted by 65,000+ professionals.</p>
      <p class="wk-foot-link">Stop losing documents you can't afford to lose: <a href="https://www.workiro.com/">visit workiro.com&nbsp;→</a></p>
...
function _printSharedStyles() {
  return `
    ...
    /* Workiro footer */
    .wk-foot { margin-top: 48px; padding-top: 22px; border-top: 1px solid #e0e0e8; text-align: left; page-break-inside: avoid; max-width: 540px; }
    .wk-foot-logo { margin-bottom: 16px; }
    /* Hook line — bold, on its own row so the eye lands here first. */
    .wk-foot-hook { font-size: 13pt; font-weight: 700; color: #1a1a2e; margin: 0 0 6px; line-height: 1.3; }
    /* Pitch paragraph — the 'Downloads folder...' rhythm. */
    .wk-foot-pitch { font-size: 10.5pt; line-height: 1.55; color: #1a1a2e; margin: 0 0 14px; }
    /* Workiro descriptor — kept un-bolded so the hook above stays the
       attention-getter and this reads as supporting context. */
    .wk-foot-cta { font-size: 10.5pt; line-height: 1.55; color: #1a1a2e; margin: 0 0 14px; }
    /* CTA line — body text with an embedded purple link for 'visit
       workiro.com'. Sentence-case, conversational, not mono-caps. */
    .wk-foot-link { font-size: 11pt; line-height: 1.55; color: #1a1a2e; font-weight: 600; margin: 0 0 14px; }
    .wk-foot-link a { color: #6f3fd1; text-decoration: none; font-weight: 700; border-bottom: 1.5px solid rgba(111,63,209,0.35); }
  ...
  return `
    ...
    <div class="autoevent-foot">
      ...
      <div class="autoevent-foot-cta">
        Want this AI-planner for your next event? <a href="/about/" target="_blank" rel="noopener">About AutoEvent&nbsp;→</a>
      </div>
...
// Re-auth form — rendered when the magic-link callback returns an
// error_code (otp_expired, access_denied, network/timeout from
// verifyOtp, etc.) or when an existing-session check finds nothing.
//
// Restyled to mirror /login/ so the page feels like a first-class
// surface rather than a dev fallback. ALL dev-driven plumbing is
// preserved unchanged:
//   - form#reauth-form / input#reauth-email / button#reauth-btn
//     / p#reauth-msg — IDs the rest of the codebase doesn't read,
//     but the contract still lives here and any future hook can
//     find them at the same paths
//   - sendMagicLink(email) called with no redirect arg → defaults
//     to /magic-link-confirm/? exactly as the prior implementation
//   - same in-flight states: "Sending…" disabled, on error
//     re-enable + show error message, on success leave button as
//     "Sent ✓" + show success message
//   - showLoading(false) still called first so the spinner clears
//
// The headline is whatever the caller passed — varies by trigger
// (otp_expired copy, decoded errDesc, the 12s timeout fallback,
// the network-error message). Long messages will wrap; that's
// expected since the user explicitly wanted error text in the
// headline regardless.
function showReauthForm(headlineMsg = '') {
  ...
  const rawHeadline  = headlineMsg || 'Let\'s get you back in.';
  const safeHeadline = rawHeadline.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  // Headlines over ~90 chars (the network-diagnostic message is
  // ~195) demote to body-paragraph styling so the layout doesn't
  // collapse around a 6-line H1. Short messages keep the big
  // serif treatment that matches the login page.
  const headlineClass = rawHeadline.length > 90 ? 'login-page-title is-long' : 'login-page-title';
  ...
  root.innerHTML = `
    <nav class="top-bar">
      ...
      <div class="top-bar-right">
        <a class="top-bar-link" href="/about/">About</a>
    ...
    <main class="login-page">
      <div class="login-page-inner">
        ...
        <h1 class="${headlineClass}">${safeHeadline}</h1>
        <p class="login-page-sub">
          Enter your email and we'll send you a link to your Accountex 2026 plan.
        </p>

        <form class="login-page-form" id="reauth-form" novalidate>
          <label class="login-page-input-label" for="reauth-email">Your email:</label>
          <div class="login-page-input-wrap">
            <svg class="login-page-input-icon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5" width="18" height="14" rx="2"/><polyline points="3 7 12 13 21 7"/></svg>
            <input
              type="email"
          ...
          <button type="submit" class="cta-primary login-page-submit" id="reauth-btn">
            Send my magic link
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
          </button>
        </form>

        <div class="login-page-msg" id="reauth-msg" style="display:none;"></div>

        <div class="login-page-divider"></div>

        <div class="login-page-secondary">
          <div class="login-page-secondary-row">
            <span class="login-page-secondary-label">Stuck or need help?</span>
            <a class="login-page-secondary-link" href="mailto:help@autoevent.io">help@autoevent.io</a>
          </div>
        </div>
    ...
    <div class="autoevent-foot">
      ...
      <div class="autoevent-foot-cta">
        Want this AI-planner for your next event? <a href="/about/">About AutoEvent&nbsp;→</a>
      </div>
      <div class="autoevent-foot-legal">
        <a href="https://www.workiro.com/terms-and-policies/autoevent" target="_blank" rel="noopener">Privacy &amp; terms</a>
      </div>
    </div>
  `;

  // Submit handler — preserves the original behaviour exactly. On
  // success the button is left disabled at "Sent ✓" so the user
  // doesn't fire repeated requests; on error it's re-enabled and an
  // error message is surfaced beneath the form.
  const form    = $('reauth-form');
  const btn     = $('reauth-btn');
  const msg     = $('reauth-msg');
  const emailIn = $('reauth-email');
  ...
  if (form) form.addEventListener('submit', async (e) => {
    ...
    if (!email || !email.includes('@')) {
      ...
      if (msg) {
        ...
        msg.className = 'login-page-msg error';
    ...
    if (error) {
      ...
      if (msg) {
        msg.style.display = 'block';
        msg.className = 'login-page-msg error';
        msg.innerHTML = `${escHtml(error.message || 'Something went wrong. Please try again.')} If this keeps happening, email <a href="mailto:help@autoevent.io">help@autoevent.io</a>.`;
      }
    } else {
      if (msg) {
        msg.style.display = 'block';
        msg.className = 'login-page-msg success';
        msg.innerHTML = `<strong>Check your inbox.</strong> A fresh magic link is on its way to <strong>${escHtml(email)}</strong>.`;
      }
...
function buildPrintHtml() {
  ...
  // ── Mission (aggregate) + per-team-member onboarding ──────────────
  // Mission section pulls two threads, aggregated across the whole
  // team where one is in play:
  //   problems     — union of every teammate's pain labels (Stage 1)
  //   tools        — union of every teammate's selected tool categories (Stage 2)
  // Rendered as the pink-accent quote (problems) plus a purple-accent
  // strip below (tools), so the reader sees both 'what hurts' and
  // 'where they want help' at a glance.
  // For solo users the eyebrow swaps to 'YOU FLAGGED' wording so the
  // copy doesn't pretend a team of one is a team.
  const teamPlansForMission = _teamData?.teamPlans?.length
    ? _teamData.teamPlans
    : (_plan ? [_plan] : []);
  const isTeamMission = teamPlansForMission.length > 1;

  const aggregateProblemsSet = new Set();
  const aggregateCategoriesSet = new Set();
  for (const p of teamPlansForMission) {
    if (p.problem) {
      p.problem.split(',').map(s => s.trim()).filter(Boolean).forEach(s => aggregateProblemsSet.add(s));
    }
    for (const c of (p.categories || [])) aggregateCategoriesSet.add(c);
  }
  const aggregateProblem = [...aggregateProblemsSet].join(', ');
  const aggregateTools   = [...aggregateCategoriesSet]
    .map(c => CATEGORY_LABELS[c] || c)
    .filter(Boolean);

  const missionEyebrow = isTeamMission
    ? 'Biggest problems your team flagged as needing to be solved this year'
    : 'Biggest problems you flagged as needing to be solved this year';
  const toolsLabel = 'Tools flagged as holding your firm back';

  const toolsHtml = aggregateTools.length
    ? `<p class="mission-tools"><span class="mission-tools-label">${esc(toolsLabel)}</span><span class="mission-tools-list">${esc(aggregateTools.join(' · '))}</span></p>`
    : '';
  const missionHtml = (aggregateProblem || aggregateTools.length) ? `
    <section class="block">
      <div class="section-eyebrow tone-pink">${esc(missionEyebrow)}</div>
      ${aggregateProblem ? `<blockquote class="mission-quote">${esc(aggregateProblem)}</blockquote>` : ''}
      ${toolsHtml}
    </section>
    <hr class="doc-divider">` : '';

  // Per-teammate onboarding answers — only when there's more than
  // one person on the plan. Useful as a discussion prompt: shows
  // each teammate's individual pains + tools side-by-side so the
  // team can spot overlap, divergence, who owns what.
  const memberCardsHtml = isTeamMission ? teamPlansForMission.map(p => {
    const isMe = p.user_id === _authUser?.id;
    const member = isMe ? null : members.find(mm => mm.users?.id === p.user_id);
    const fnPart   = isMe ? (firstName || 'You')        : (member?.users?.first_name || 'Teammate');
    const lnPart   = isMe ? lastName                    : (member?.users?.last_name  || '');
    const coPart   = isMe ? company                     : (member?.users?.company    || '');
    const fullName = [fnPart, lnPart].filter(Boolean).join(' ') || 'Teammate';
    const roleLbl  = p.role && ROLE_LABELS[p.role] ? ROLE_LABELS[p.role] : '';
    const meta     = [coPart, roleLbl].filter(Boolean).join(' · ');
    const memberPains = p.problem || '—';
    const memberTools = (p.categories || [])
      .map(c => CATEGORY_LABELS[c] || c)
      .filter(Boolean)
      .join(', ') || '—';
    return `
      <div class="member-card">
        <h4 class="member-name">${esc(fullName)}${isMe ? ' <span class="member-you">(you)</span>' : ''}</h4>
        ${meta ? `<p class="member-meta">${esc(meta)}</p>` : ''}
        <p class="member-line"><span class="member-label tone-pink">Pains</span><span class="member-value">${esc(memberPains)}</span></p>
        <p class="member-line"><span class="member-label tone-purple">Tools</span><span class="member-value">${esc(memberTools)}</span></p>
      </div>`;
  }).join('') : '';

  const teamOnboardingHtml = memberCardsHtml ? `
    <section class="block">
      <div class="section-eyebrow tone-pink">Team onboarding · individual answers</div>
      <h2 class="section-title">Where each teammate said the firm needs help.</h2>
      <p class="empty-state-note">A great prompt for a team debrief — discuss each line, see what overlaps, decide who owns what.</p>
...
<style>
  ...
  /* Tools line beneath the mission quote — mono-caps label + purple
     comma-list so it visually maps to the booth/category palette
     used elsewhere in the app. */
  .mission-tools { margin: 12px 0 0; padding: 10px 14px 10px 18px; border-left: 3px solid #a855f7; background: rgba(168,85,247,0.05); border-radius: 0 6px 6px 0; font-size: 10pt; line-height: 1.55; color: #1a1a2e; }
  .mission-tools-label { font-family: 'JetBrains Mono', 'SF Mono', Menlo, monospace; font-size: 8.5pt; letter-spacing: 0.14em; text-transform: uppercase; font-weight: 700; color: #6f3fd1; margin-right: 10px; }
  .mission-tools-list { color: #1a1a2e; font-weight: 500; }

  /* Per-teammate onboarding cards — one per team member with their
     individual pains + tools side-by-side. Designed as a debrief
     discussion prompt, so each card is its own self-contained block
     with a serif name and mono-caps meta line for credibility. */
  .empty-state-note { font-size: 10pt; line-height: 1.55; color: #6b6b80; margin: 0 0 14px; max-width: 540px; font-style: italic; }
  .member-grid { display: flex; flex-direction: column; gap: 10px; }
  .member-card { padding: 14px 16px; border: 1px solid #e0e0e8; border-radius: 8px; background: #fff; page-break-inside: avoid; }
  .member-name { margin: 0 0 4px; font-family: Georgia, serif; font-weight: 700; font-size: 12.5pt; color: #0a0a12; line-height: 1.3; }
  .member-name .member-you { font-family: 'JetBrains Mono', 'SF Mono', Menlo, monospace; font-size: 7.5pt; letter-spacing: 0.14em; text-transform: uppercase; color: #0fb88a; font-weight: 700; margin-left: 6px; vertical-align: middle; }
  .member-meta { margin: 0 0 10px; font-family: 'JetBrains Mono', 'SF Mono', Menlo, monospace; font-size: 8.5pt; letter-spacing: 0.10em; text-transform: uppercase; color: #6b6b80; font-weight: 700; }
  .member-line { margin: 0 0 5px; font-size: 10pt; line-height: 1.5; color: #1a1a2e; }
  .member-line:last-child { margin: 0; }
  .member-label { display: inline-block; font-family: 'JetBrains Mono', 'SF Mono', Menlo, monospace; font-size: 7.5pt; letter-spacing: 0.14em; text-transform: uppercase; font-weight: 700; margin-right: 8px; min-width: 44px; }
  .member-label.tone-pink   { color: #ff5e84; }
  .member-label.tone-purple { color: #6f3fd1; }
  .member-value { color: #1a1a2e; }
